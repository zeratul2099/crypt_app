...
def rsaDecrypt(request):
    try:
        ...
        return PrivKey.private_decrypt(
            number.long_to_bytes(request.POST["cypher_text"]),
            1
        ).replace('\0', '')
    except RSA.RSAError as e:
        return str(e)
...
def affineDecrypt(request):
    return affineDec(
        request.POST["cypher_text"],
        int(request.POST['keyA']),
        int(request.POST['keyB'])
    )
